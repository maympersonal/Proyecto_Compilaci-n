...
class HulkParser(Parser):
    ...
    def var_dec(self, p):
        ...
        return insert_error_info(p,VarDeclaration(p.var_init_list, p.var_decl_expr))
    ...
    def var_init(self, p):
        ...
        if len(p) == 3:
            return insert_error_info(p,VarInit(p.identifier, p.inst, p[1]))
        else:
            return insert_error_info(p,VarInit(p.identifier, p.inst, p[1],p.type_downcast))
    ...
    def fully_typed_param(self, p):
        ...
        return insert_error_info(p,VarUse(p.IDENTIFIER, p.type_anotation))
    ...
    def scope(self, p):
        ...
        if len(p)==3:
            return insert_error_info(p,Scope(p[1]))
    ...
    def expression(self, p):
        ...
        return insert_error_info(p,Concat(p[1], p[0], p[2]))
    ...
    def aritmetic_operation(self, p):
        ...
        if len(p)==1:
            ...
            return insert_error_info(p,Add(p[0],p[2]))
        elif p[1]=='-':
            return insert_error_info(p,Sub(p[0],p[2]))
    ...
    def term(self, p):
        ...
        if len(p)==1:
            ...
            return insert_error_info(p,Mult(p[0],p[2]))
        elif p[1]=='/':
            return insert_error_info(p,Div(p[0],p[2]))
        elif p[1]=='%':
            return insert_error_info(p,Mod(p[0],p[2]))  
    ...
    def factor(self, p):
        ...
        return insert_error_info(p,Power(p[0],p[2]))
    ...
    def factor(self, p):
        self.parsertrack.append("factor "+str([v for v in p]))
        return insert_error_info(p,Unary(p[0], p[1]))
    ...
    def atom(self, p):
        ...
        return insert_error_info(p,String(p[0]))
        
    @_('NUMBER')
    def atom(self, p):
        self.parsertrack.append("Number "+str([v for v in p]))
        return insert_error_info(p,Number(p[0]))
    ...
    def function_declaration(self, p):
        ...
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_full_declaration, parameters = p.parameters))
        
    @_('FUNCTION IDENTIFIER LPAREN RPAREN function_full_declaration')
    def function_declaration(self, p):
        self.parsertrack.append("function_declaration "+str([v for v in p]))
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_full_declaration))
    ...
    def function_declaration(self, p):
        self.parsertrack.append("function_declaration "+str([v for v in p]))
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_inline_declaration, parameters = p.parameters))
    ...
    def function_declaration(self, p):
        self.parsertrack.append("function_declaration "+str([v for v in p]))
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_full_declaration, type_anotation = p.type_anotation, parameters = p.parameters))
        
    @_('FUNCTION IDENTIFIER LPAREN RPAREN type_anotation function_full_declaration')
    def function_declaration(self, p):
        self.parsertrack.append("function_declaration "+str([v for v in p]))
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_full_declaration, type_anotation = p.type_anotation))
    ...
    def function_declaration(self, p):
        self.parsertrack.append("function_declaration "+str([v for v in p]))
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_inline_declaration, type_anotation = p.type_anotation, parameters = p.parameters))
        
    @_('FUNCTION IDENTIFIER LPAREN RPAREN type_anotation function_inline_declaration')
    def function_declaration(self, p):
        self.parsertrack.append("function_declaration "+str([v for v in p]))
        return insert_error_info(p,FunctionDeclaration(p.IDENTIFIER, p.function_inline_declaration, type_anotation = p.type_anotation ))
    ...
    def while_loop(self, p):
        ...
        return insert_error_info(p,WhileLoop(p[2], p[4]))
    ...
    def for_loop(self, p):
        ...
        return insert_error_info(p,ForLoop(p[2], p[4] , p[6]))

    # Expresión condicional
    @_('condition AND conditional_expression', 
       'condition OR conditional_expression', 
       'NOT condition', 
       'condition CAND conditional_expression', 
       'condition COR conditional_expression', 
       'CNOT condition', 
       'condition')
    def conditional_expression(self, p):
        ...
        if len(p)==1:
            ...
            return insert_error_info(p,And(p[0],p[2]))
        elif p[1]=='|':
            return insert_error_info(p,Or(p[0],p[2]))
        elif p[0]=='!':
            return insert_error_info(p,Not(p[1]))
    ...
    def condition(self, p):
        self.parsertrack.append("condition "+str([v for v in p]))
        return insert_error_info(p,Is(p[0],p[2]))
    ...
    def comparation(self, p):
        ...
        if p[1]=='>':
            return insert_error_info(p,GreaterThan(p[0],p[2]))
        elif p[1]=='<':
            return insert_error_info(p,LessThan(p[0],p[2]))
        elif p[1]=='>=':
            return insert_error_info(p,GreaterEqual(p[0],p[2]))  
        elif p[1]=='<=':
            return insert_error_info(p,LessEqual(p[0],p[2]))
        elif p[1]=='==':
            return insert_error_info(p,Equal(p[0],p[2]))
        elif p[1]=='!=':
            return insert_error_info(p,NotEqual(p[0],p[2]))
    ...
    def boolean_value(self, p):
        ...
        return insert_error_info(p,Boolean(p[0]))
    ...
    def type_declaration(self, p):
        ...
        return insert_error_info(p,TypeDeclaration(p.IDENTIFIER, parameters = p.parameters,decl_body = p.decl_body))
        
    @_( 'TYPE IDENTIFIER parameters inherits_type decl_body',
        'TYPE IDENTIFIER parameters inherits_type decl_body SEMICOLON')
    def type_declaration(self, p):
        self.parsertrack.append("type_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeDeclaration(p.IDENTIFIER, parameters = p.parameters, inherits_type = p.inherits_type, decl_body = p.decl_body))
            
    @_('TYPE IDENTIFIER decl_body',
       'TYPE IDENTIFIER decl_body SEMICOLON')
    def type_declaration(self, p):
        self.parsertrack.append("type_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeDeclaration(p.IDENTIFIER, decl_body = p.decl_body))
        
    @_('TYPE IDENTIFIER inherits_type decl_body',
       'TYPE IDENTIFIER inherits_type decl_body SEMICOLON')
    def type_declaration(self, p):
        self.parsertrack.append("type_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeDeclaration(p.IDENTIFIER, inherits_type = p.inherits_type, decl_body = p.decl_body))
    ...
    def inherits_type(self, p):
        ...
        if len(p) == 3:
            return insert_error_info(p,InheritsType(p.IDENTIFIER, p.parameters))
        else:
            return insert_error_info(p,InheritsType(p.IDENTIFIER))
    ...
    def decl_body(self, p):
        ...
        if len(p)==3:
            return insert_error_info(p,DeclarationScope(p[1]))
        else:
            return insert_error_info(p,DeclarationScope([]))
    ...
    def atribute_declaration(self, p):
        ...
        if len(p) == 3:
            return insert_error_info(p,TypeVarInit(p.identifier, p.expression))
        else:
    ...
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.function_full_declaration, parameters = p.parameters))
        
    @_('IDENTIFIER LPAREN RPAREN RETURN expression')
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.expression))
    ...
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.expression, type_anotation = p.type_anotation, parameters = p.parameters))
        
    @_('IDENTIFIER parameters type_anotation function_full_declaration')
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.function_full_declaration, type_anotation = p.type_anotation, parameters = p.parameters))
        
    @_('IDENTIFIER LPAREN RPAREN type_anotation RETURN expression')
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.expression, type_anotation = p.type_anotation))
        
    @_('IDENTIFIER LPAREN RPAREN type_anotation RETURN conditional_expression')
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.conditional_expression, type_anotation = p.type_anotation))
        
    @_('IDENTIFIER LPAREN RPAREN type_anotation function_full_declaration')
    def method_declaration(self, p):
        self.parsertrack.append("method_declaration "+str([v for v in p]))
        return insert_error_info(p,TypeMethodDeclaration(p.IDENTIFIER, p.function_full_declaration, type_anotation = p.type_anotation))
    ...
    def function_call(self, p):
        ...
        if len(p) == 4:
            return insert_error_info(p,FunctionCall(p[0], p[2]))
        else:
            return insert_error_info(p,FunctionCall(p[0]))
    ...
    def type_instanciation(self, p):
        ...
        if len(p) == 5:
            return insert_error_info(p,TypeInstanciation(p.IDENTIFIER, p.arguments_list))
        else:
            return insert_error_info(p,TypeInstanciation(p.IDENTIFIER))
    ...
    def var_use(self, p):
        ...
        return insert_error_info(p, VarUse(p[0]))
    
    @_('atom LBRACKET expression RBRACKET')
    def var_use(self, p):
        self.parsertrack.append("var_use "+str([v for v in p]))
        return insert_error_info(p,VectorVarUse(p[0], p[2]))
    ...
    def var_attr(self, p):
        ...
        return insert_error_info(p,VarAttr(p[0], p[2]))
    ...
    def var_method(self, p):
        ...
        return insert_error_info(p,VarMethod(p.IDENTIFIER, p.function_call))
    ...
    def virtual_method(self, p):
        ...
        return insert_error_info(p,ProtocolMethodDeclaration(p[0], p[3]))
    
    @_('IDENTIFIER parameters type_anotation')
    def virtual_method(self, p):
        self.parsertrack.append("virtual_method "+str([v for v in p]))
        return insert_error_info(p,ProtocolMethodDeclaration(p[0], p[2], p[1]))
    ...
    def vector_decl(self, p):
        ...
        return insert_error_info(p,VectorRangeDeclaration(p[0]))
    
    @_('expression SINCETHAT identifier IN expression',
       'expression OR identifier IN expression')
    def vector_decl(self, p):
        self.parsertrack.append("vector_decl "+str([v for v in p]))
        return insert_error_info(p,VectorExpressionDeclaration(p[0], p[2], p[4]))
    ...
    def build_in_range(self, p):
        ...
        return insert_error_info(p,BinaryBuildInFunction(p[0], p[2], p[4]))

    # Imprimir
    @_('PRINT LPAREN argument RPAREN')
    def build_in_print(self, p):
        self.parsertrack.append("build_in_print "+str([v for v in p]))
        return insert_error_info(p,UnaryBuildInFunction(p[0], p[2]))
    ...
    def build_in_functions(self, p):
        self.parsertrack.append("build_in_functions "+str([v for v in p]))
        return insert_error_info(p,NoParamBuildInFunction(p[0]))
    ...
    def build_in_consts(self, p):
        ...
        return insert_error_info(p,BuildInConst(p[0]))
    ...
    def error(self, p):
        ...
        self.errors.append(f'{lineno}: Syntax error at {value} {p}')
